...
<?php require_once 'header.php'; ?>
 ...
 <!-- Nav Sidebar -->
    ...
    <div class="large-9 columns">
    	<div class="panel"><h3>Find people on Friendbook</h3>
    ...
    <?php 
    	$rs = mysqli_query($conn, "SELECT PageName, User_ID, (SELECT COUNT(User1) 
    	FROM User_Follows_User WHERE User2 IN (SELECT User2 FROM User_Follows_User 
    	WHERE User1 = $user_id AND User2 != User_ID) AND User1 = User_ID AND User2 != $user_id) AS Mutual_Friends FROM User_Page WHERE PageName LIKE '%$search%' ORDER BY Mutual_Friends DESC");

		while( $row = mysqli_fetch_array($rs, MYSQL_ASSOC) )
		{
			if((strcmp($user_id, $row['User_ID']))){
			echo '<div class="row">
      		<div class="large-2 columns small-3"><img src="/img/80x80.jpg" /></div>
      		<div class="large-10 columns">';
			echo '<h4><a href="/page.php?ID=' . $row['User_ID'] . '">' . $row['PageName'] . '</a><span data-tooltip class="has-tip right" title="';
			$whoFriendQuery = "SELECT PageName FROM User_Page WHERE User_ID IN (SELECT (User2) FROM User_Follows_User WHERE User2 IN (SELECT User2 FROM User_Follows_User WHERE User1 = $user_id AND User2 != ". $row['User_ID'] .") AND User1 = ". $row['User_ID'] ." AND User2 != $user_id)";
